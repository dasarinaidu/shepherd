// Package oidc provides primitives for interacting with Rancher's OIDC provider:
// resource GVK identifiers, well-known endpoint paths, the token-response data
// shape, and helpers for fetching the discovery document and exchanging refresh
// tokens. It contains no test config, defaults, or workflow logic.
package oidc
...
// OIDCClient resource GroupVersionKind identifiers.
const (
...
// Rancher OIDC and v3 API endpoint paths.
const (
...
// TokenSet is the OIDC token-endpoint response from a PKCE auth-code or refresh exchange.
...
// Discovery fetches the OIDC provider metadata document. rancherURL must include scheme
...
// RefreshAccessToken exchanges a refresh_token for a new TokenSet. rancherURL must include
// scheme and have no trailing slash.
