...
// OIDCClientSecretNamespace is the canonical Rancher namespace where OIDCClient
// resource secrets are stored.
...
// CreateOIDCClient creates an OIDCClient CRD on the management cluster from the
// caller-provided spec, registers DeleteOIDCClient as a session cleanup function,
...
// WaitForOIDCClientReady polls until the OIDCClient's status.clientID and
// status.clientSecrets fields are populated. Returns the clientID and one of the
// secret key names.
...
// FetchOIDCClientSecret retrieves the OIDCClient secret value from the canonical
// Rancher OIDCClient secrets namespace.
...
// DeleteOIDCClient deletes the OIDCClient resource by name. NotFound is treated
// as success so the function is safe to call from cleanup paths.
