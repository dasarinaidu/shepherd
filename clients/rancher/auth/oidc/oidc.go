...
// APIClient is the OIDC auth provider client. It loads the "oidc" config block,
// holds two http.Clients (one redirect-following, one not), and exposes OIDC-endpoint methods used by tests.
...
// NewAPIClient constructs an APIClient against the given Rancher URL. It loads the "oidc" config key and forces InsecureSkipVerify on the underlying TLS transport.
...
// RawRequest executes an HTTP request against a Rancher API path using the client's TLS-skip transport.
...
// CompleteAuthCodeFlow drives the headless PKCE authorization-code flow end to end and returns a TokenSet.
// Retries up to 5 times if the auth endpoint redirects to the dashboard login page.
...
// Used by negative-path security tests that exercise Rancher's resilience to malformed JWT headers.
